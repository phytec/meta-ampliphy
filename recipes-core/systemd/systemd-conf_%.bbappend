...
SRC_URI:append:phyflex-phyvip = " file://37-can-imx-soc.rules"
...
do_install:append() {
    ...
    for file in $(find ${UNPACKDIR} -maxdepth 1 -type f -name "[0-9][0-9]-*.network"); do
        install -m 0644 "$file" ${D}${systemd_unitdir}/network/
    done
    for file in $(find ${UNPACKDIR} -maxdepth 1 -type f -name "[0-9][0-9]-*.link"); do
        install -m 0644 "$file" ${D}${systemd_unitdir}/network/
    done
    ...
    if [ -e ${UNPACKDIR}/30-stamped-fpsc-devnames-imx93.rules ]; then
        install -d ${D}${sysconfdir}/udev/rules.d
        install -m 0644 ${UNPACKDIR}/30-stamped-fpsc-devnames-imx93.rules ${D}${sysconfdir}/udev/rules.d/
    fi
